...
process CleanFiles {
    publishDir params.outdir, pattern: "*locus_count.txt"
    tag {reads.baseName}
    ...
    output:
    path "${reads.baseName.replace('.fastq', '')}_clean.fastq", emit: cleandata_dir

    script:
    // Call Clean_nano_data_simple_nf_core_version.py script with the reads, and output directory
    """
    Clean_nano_data_simple_nf_core_version.py -i $reads -p $params.primers -o "${reads.baseName.replace('.fastq', '')}_clean.fastq" -j "${reads.baseName.replace('.fastq', '')}_clean_junk.fastq" -c "${reads.baseName.replace('.fastq', '')}_locus_count.txt"
    """
}
...
process Demultiplex {
    ...
    output:
    path "${file.baseName.replace('.fastq', '')}_demultiplexed_output", emit: demultiplexed_dir

    script:
    // Call demultiplex.py script with the reads, primers, and output directory
    """
    demultiplex.py $file $params.primers "${file.baseName.replace('.fastq', '')}_demultiplexed_output"
    """
}
...
process Typing {
    publishDir params.outdir, pattern: "*types.res"
...
process AggregateResults {
    publishDir params.outdir, pattern: "*aggregated.txt"
    tag { fileName }
...
workflow{
    ...
    // CleanFiles
    cleandata = CleanFiles(reads_ch)  
    cleandata_files_ch = cleandata.flatMap { dir ->
